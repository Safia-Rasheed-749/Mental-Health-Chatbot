...
Model: roberta-base (depression_v2)
Dataset: Depression V2 (70/15/15 split)
Task: Binary classification (Depression vs No Depression)

This script evaluates the trained V2 model on the reserved
test set (test.csv) and generates:
...
# Output directory
OUTPUT_DIR = BASE_DIR / "evaluation" / "depression_v2"
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("=" * 70)
print("RoBERTa Depression V2 Model – Final Test Evaluation")
print("=" * 70)
...
# Try loading from config or define manually
label_mapping = {
    "0": "No Depression",
    "1": "Depression"
}

# Check if training_config.json exists to get label info
...
plt.title("Depression V2 Model - Confusion Matrix (Test Set)", fontsize=14, fontweight='bold')
plt.xlabel("Predicted Label", fontsize=12)
plt.ylabel("Actual Label", fontsize=12)
plt.tight_layout()

confusion_matrix_file = OUTPUT_DIR / "depression_v2_confusion_matrix.png"
...
predictions_file = OUTPUT_DIR / "depression_v2_predictions.csv"
...
wrong_predictions_file = OUTPUT_DIR / "depression_v2_wrong_predictions.csv"
...
confidence_file = OUTPUT_DIR / "depression_v2_confidence_analysis.txt"
with open(confidence_file, "w", encoding="utf-8") as f:
    f.write("Confidence Analysis - Depression V2 Model\n")
    f.write("=" * 60 + "\n\n")
...
metrics_file = OUTPUT_DIR / "depression_v2_metrics.txt"
with open(metrics_file, "w", encoding="utf-8") as f:
    f.write("=" * 70 + "\n")
    f.write("DEPRESSION V2 MODEL - FINAL TEST EVALUATION\n")
    f.write("=" * 70 + "\n\n")
    ...
    f.write(f"Fine-tuned Model: depression_v2\n")
    f.write(f"Task: Binary Classification (Depression vs No Depression)\n")
    f.write(f"Number of Classes: 2\n")
    f.write(f"Classes: 0 = No Depression, 1 = Depression\n")
